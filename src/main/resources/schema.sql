...
CREATE TABLE IF NOT EXISTS `CALENDAR_EVENT` (
	...
	`CALENDAR_EVENT_UPDATED_AT`	DATETIME	NULL,
...
CREATE TABLE IF NOT EXISTS `ATTENDANCE_REQUEST` (
	...
	`ATTENDANCE_REQUEST_UPADATED_AT`	DATETIME	NULL
);
...
CREATE TABLE IF NOT EXISTS `KANBAN_CARD` (
	...
	`TASK_STATUS`	VARCHAR(20)	NOT NULL	COMMENT '해야 할 일 / 진행중 / 완료 (TODO / DOING / DONE)',
	`TASK_CREATED_AT`	DATETIME	NOT NULL,
	`TASK_UPDATED_AT`	DATETIME	NULL,
	`PROJECT_MEMBER_NO`	BIGINT	NOT NULL,
	`TASK_STARTED_AT`	DATE	NULL,
	`TASK_ENDED_AT`	DATE	NULL
);
...
CREATE TABLE IF NOT EXISTS `TASK_HISTORY` (
	...
	`TASK_HISTORY_UPDATED_AT`	DATETIME	NULL
);
...
CREATE TABLE IF NOT EXISTS `HEALTH_SURVEY_RESPONSE` (
	...
	`HEALTH_SURVEY_RESPONSE_CREATED_AT`	DATETIME	NULL
);
...
ALTER TABLE `KANBAN_CARD` ADD CONSTRAINT `PK_KANBAN_CARD` PRIMARY KEY (`TASK_NO`);
...
ALTER TABLE `KANBAN_CARD` MODIFY `TASK_NO` BIGINT NOT NULL AUTO_INCREMENT;
...
-- ============================================
-- 4단계: 외래키 제약조건 추가 (AUTO_INCREMENT 적용 후)
-- ============================================
-- AGENCY 관련
...
-- KANBAN_CARD 관련
ALTER TABLE `COMMENT` ADD CONSTRAINT `FK_COMMENT_TASK` FOREIGN KEY (`TASK_NO`) REFERENCES `KANBAN_CARD`(`TASK_NO`);
ALTER TABLE `TASK_HISTORY` ADD CONSTRAINT `FK_TASK_HISTORY_TASK` FOREIGN KEY (`TASK_NO`) REFERENCES `KANBAN_CARD`(`TASK_NO`);
