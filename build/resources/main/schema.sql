...
CREATE TABLE IF NOT EXISTS `HEALTH_SURVEY` (
	...
	`AGENCY_NO`	BIGINT	NOT NULL,
	`HEALTH_SURVEY_PERIOD`	INT	DEFAULT 15,
	`HEALTH_SURVEY_CYCLE`	INT	DEFAULT 30,
...
CREATE TABLE IF NOT EXISTS `KANBAN_CARD` (
	...
	`KANBAN_CARD_DESCRIPTION`	VARCHAR(1000)	NULL	COMMENT '작업 내용 설명',
	`KANBAN_CARD_STATUS`	VARCHAR(1)  DEFAULT 'N'	NOT NULL	COMMENT 'Y: 완료, N: 미완료, D: 삭제(숨김)',
...
CREATE TABLE IF NOT EXISTS `HEALTH_SURVEY_RESPONSE_ITEM` (
	...
	`MEMBER_NO`	BIGINT	NOT NULL,
	`HEALTH_SURVEY_QUESTION_NO`	BIGINT	NOT NULL,
	`HEALTH_SURVEY_QUESTION_ITEM_ANSWER_SCORE`	INT	NOT NULL	COMMENT '각 문항별 점수의 총합 (프론트엔드에서 계산)',
	`HEALTH_SURVEY_QUESTION_ITEM_CREATED_AT`	DATETIME    NOT NULL    DEFAULT CURRENT_TIMESTAMP	COMMENT '생성일 유무로 검진 완료 여부 판단'
);
